...
class CheckoutActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (mAddressDetails != null) {
            tv_checkout_address_type_checkout_activity.text = mAddressDetails?.type
            ...
            tv_checkout_additional_note_checkout_activity.text = mAddressDetails?.additionalNote

            if (mAddressDetails?.otherDetails!!.isNotEmpty()) {
                tv_checkout_other_details_checkout_activity.text = mAddressDetails?.otherDetails
            }
            tv_checkout_mobile_number_checkout_activity.text = mAddressDetails?.mobileNumber
        }
    ...
    fun successCartItemsList(cartList: ArrayList<Cart>) {
        ...
        tv_checkout_sub_total_checkout_activity.text = "$$mSubTotal"
        // Here we have kept Shipping Charge is fixed as $10 but in your case it may cary. Also, it depends on the location and total amount.
        tv_checkout_shipping_charge_checkout_activity.text = "$10.0"

        if (mSubTotal > 0) {
            ll_checkout_place_order_checkout_activity.visibility = View.VISIBLE

            mTotalAmount = mSubTotal + 10.0
            tv_checkout_total_amount_checkout_activity.text = "$$mTotalAmount"
        } else {
